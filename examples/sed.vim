...
syn region sedAddress   matchgroup=Special start="[{,;]\s*/\(\\/\)\="lc=1 skip="[^\\]\(\\\\\)*\\/" end="/[IM]\=" contains=sedTab,sedRegexpMeta
syn region sedAddress   matchgroup=Special start="^\s*/\(\\/\)\=" skip="[^\\]\(\\\\\)*\\/" end="/[IM]\=" contains=sedTab,sedRegexpMeta
syn match sedComment	"^\s*#.*$"
syn match sedFunction	"[dDgGhHlLnNpPxz=]\s*\($\|;\)" contains=sedSemicolon,sedWhitespace
syn match sedLabel	":[^;]*"
...
" QUIT
syn region sedQuit	matchgroup=sedFunction start="[qQv]" matchgroup=sedSemicolon end=";\|$" contains=sedWhitespace
syn region sedBranch	matchgroup=sedFunction start="[btT]" matchgroup=sedSemicolon end=";\|$" contains=sedWhitespace
syn region sedRW	matchgroup=sedFunction start="[rwRW]" matchgroup=sedSemicolon end=";\|$" contains=sedWhitespace
...
syn match sedFlag	    "[[:digit:]gpIeimM]*w\=" contains=sedFlagwrite contained
